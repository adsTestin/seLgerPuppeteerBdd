...
const deleteCookie = require("./action/deleteCookie");
// Process .env file
...
Before(async function(scenario) {
  ...
  const new_feature = featureScope.isNewFeature(currentFeature);
  ...
  console.log(scenario.sourceLocation.uri, "New Feature : "+new_feature)
  // Use the same page object for each scenario in a feature

  if (new_feature) await openUrl.call(this,config.rootUrl);
});
...
After(async function(scenario) {
  ...
  if (scenario.result.status === Status.FAILED) {
    ...
    featureScope.browserScope.attach(screenshot,'image/png');
  }
  //await deleteCookie.call(this, "all");
  const url = await this.page.url();
  if(url != config.rootUrl){ await this.page.goto(config.rootUrl)}
});
